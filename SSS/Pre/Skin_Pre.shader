Shader "SSS/Skin_Pre" 
{
    ...
    {         
    	...
    	//_LutY("Lut Y", Range(0.01, 1)) = 0.001
		_CurveFactor("CurveFactor",Range(0,5)) = 0.006
    ...
    SubShader 
    {
		...
		Pass 
		{
			...
			half3 BentNormalsDiffuseLighting(float3 normal, float3 L, float Curvature)
			{
				...
				//return tex2Dlod(_SSSLUT, float4(lerp(0.002, 0.998, (NdotLBlurredUnclamped * 0.5 + 0.5)), Curvature * _LutY, 0, 0)); // _SSSLUT的y轴翻转用这个
			...
			half4 frag_new(v2f_new i) : SV_Target
			{
				...
				//diffuse
				float3 directDiffuse = BentNormalsDiffuseLighting(worldBump, lightDir, curvature);
				...
				return float4(float3(curvature, curvature, curvature) * 0.3, 1);
			}
			ENDCG
		}
    }
