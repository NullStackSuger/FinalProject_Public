...
#include "Packages/com.unity.postprocessing/PostProcessing/Shaders/StdLib.hlsl"
            ...
            float sampleDensity(float3 rayPos) 
            {
                ...
                float heightGradient2 =
                    saturate(remap(heightPercent, 0.0, weatherMap.r, 1, 0)) *
                    saturate(remap(heightPercent, 0.0, gMin, 0, 1));
            ...
            float3 lightmarch(float3 position ,float dstTravelled)
            {
                ...
                for (int step = 0; step < 8; step++) //灯光步进次数
                {
                    ...
                    // 这里和Frag里计算密度是一样的, 只是totalDensity是e的指数部分, 而Frag里是直接计算了
                    totalDensity += max(0, sampleDensity(position) * stepSize);
                }
                float transmittance = exp(-totalDensity * _lightAbsorptionTowardSun);

                //将从亮到暗映射为 3段颜色 ,亮->灯光颜色 中->ColorA 暗->ColorB
                ...
                return transmittance * cloudColor;//_darknessThreshold + transmittance * (1 - _darknessThreshold) * cloudColor;
            }
            ...
            {
                // "_CameraDepthTexture_TexelSize"1个像素的大小
                ...
                // sampler_CameraDepthTexture是采样器, 控制如何采样(如线性采样, 寻址模式(uv>1怎么采样)等)
